...
publiczny SetName void | Ustawia nazwę zasady.
publiczny const std::string & GetDescription | Pobiera opis zasad.
publiczny Opis_zestawu void | Ustawia opis zasad.
publiczny std::vector const < UserRights > & GetUserRightsList | Pobiera kolekcję mapowań prawa użytkownika.
...
publiczny SetContentValidUntil void | Ustawia czas wygaśnięcia zasad.
publiczny bool DoesAllowOfflineAccess | Pobiera czy zasada zezwala na dostęp do zawartości w trybie offline.
publiczny SetAllowOfflineAccess void | Ustawia, czy w trybie offline umożliwia zasad dostęp do zawartości.
publiczny const std::string & GetReferrer | Pobiera adres odwołania zasad.
publiczny SetReferrer void | Ustawia adres odwołania zasad.
publiczny const AppDataHashMap & GetEncryptedAppData | Pobiera dane specyficzne dla aplikacji, która została zaszyfrowana.
Wartość & publicznego SetEncryptedAppDataAppDataHashMap void) | Ustawia dane specyficzne dla aplikacji, które powinny być szyfrowane.
publiczny const AppDataHashMap & GetSignedAppData | Pobiera dane specyficzne dla aplikacji, który został podpisany.
Wartość & publicznego SetSignedAppDataAppDataHashMap void) | Ustawia dane specyficzne dla aplikacji, które powinny być podpisane.
